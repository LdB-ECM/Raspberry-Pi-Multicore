...
static __attribute__((aligned(TLB_ALIGNMENT))) VMSAv8_64_DESCRIPTOR Stage2virtual[512] = { 0 };
...
void MMU_setup_pagetable (void)
{
	...
	/* First set some default values for all 4GB */
	/* APX = 0	AP[1:0] = b11 so read/write for privilege and user and cache_disbaled */
	/* You could refine this to read only on .txt .rodata sections if required */
	for (i = 0; i < NUM_PAGE_TABLE_ENTRIES; i++) {
		page_table_map1to1[i] = i << 20 | (3 << 10) | MT_CACHE_DISABLED;
	}
	...
	for (i = 0; i < msg[3]; i++) {
		page_table_map1to1[i] = i << 20 | (3 << 10) | MT_CACHE_WRITEBACK;
	}
#endif
